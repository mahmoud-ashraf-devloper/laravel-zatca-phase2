...
<head>
    <meta charset="utf-8">
    <title>CREDIT NOTE #{{ $data['invoice_number'] }}</title>
    <style>
        body {
...
<div class="header">
    <h1>CREDIT NOTE</h1>
    <p>Credit Note #: {{ $data['invoice_number'] }}</p>
...
@if(!empty($data['original_invoice_number']))
    <div class="reference">
        <strong>Reference:</strong> This credit note refers to original invoice #{{ $data['original_invoice_number'] }}
    </div>
@endif
        ...
        @if(isset($document->items) && count($document->items) > 0)
            @foreach($document->items as $item)
                <tr>
                    ...
                    <td>{{ number_format(abs($item->tax_amount ?? 0), 2) }} SAR</td>
                    <td>{{ number_format(abs($item->total_amount ?? 0), 2) }} SAR</td>
                </tr>
            @endforeach
        @else
            <tr>
...
<div class="footer">
    <p>This is a system-generated credit note and is valid without a signature.</p>
